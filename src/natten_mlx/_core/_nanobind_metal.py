...
from . import pure
from ._forward_tuning import FORWARD_SOFTMAX_STRATEGY_TABLE, FORWARD_THREADGROUP_TABLE
from ._metal_sources import (
    ...
    source_2d_qk_backward_q_vec4,
    source_2d_qk_backward_k,
    source_2d_qk_backward_k_inverse,
    source_3d_av,
    source_3d_av_vec4,
...
_AV_1D_VEC4_KERNELS: dict[int, Callable] = {}
_QK_2D_KERNELS: dict[int, Callable] = {}
_QK_2D_VEC4_KERNELS: dict[int, Callable] = {}
_AV_2D_KERNELS: dict[int, Callable] = {}
_AV_2D_VEC4_KERNELS: dict[int, Callable] = {}
_QK_3D_KERNELS: dict[int, Callable] = {}
_QK_3D_VEC4_KERNELS: dict[int, Callable] = {}
_AV_3D_KERNELS: dict[int, Callable] = {}
_AV_3D_VEC4_KERNELS: dict[int, Callable] = {}
...
_FUSED_2D_KERNELS: dict[tuple[int, str], Callable] = {}
_FUSED_2D_VEC4_KERNELS: dict[tuple[int, str], Callable] = {}
_FUSED_2D_CAUSAL_KERNELS: dict[tuple[int, int, int, str], Callable] = {}
_FUSED_2D_CAUSAL_VEC4_KERNELS: dict[tuple[int, int, int, str], Callable] = {}
_FUSED_3D_KERNELS: dict[tuple[int, str], Callable] = {}
_FUSED_3D_VEC4_KERNELS: dict[tuple[int, str], Callable] = {}
_FUSED_3D_CAUSAL_KERNELS: dict[tuple[int, int, int, int, str], Callable] = {}
_FUSED_3D_CAUSAL_VEC4_KERNELS: dict[tuple[int, int, int, int, str], Callable] = {}
...
_QK_BWD_K_INV_2D_KERNELS: dict[int, Callable] = {}
_QK_BWD_Q_2D_KERNELS: dict[int, Callable] = {}
_QK_BWD_Q4_2D_KERNELS: dict[int, Callable] = {}
...
_QK_BWD_K_INV_3D_KERNELS: dict[int, Callable] = {}
_QK_BWD_Q_3D_KERNELS: dict[int, Callable] = {}
_QK_BWD_Q4_3D_KERNELS: dict[int, Callable] = {}
...
_FORWARD_SOFTMAX_STRATEGY_OVERRIDE = os.getenv("NATTEN_MLX_FUSED_SOFTMAX_STRATEGY", "auto").strip().lower()
...
def _dtype_class(dtype: mx.Dtype) -> str:
    return "lowp" if _is_low_precision_dtype(dtype) else "fp32"


def _normalize_softmax_strategy(strategy: str) -> str:
    s = str(strategy).strip().lower()
    if s not in {"stored", "recompute"}:
        raise ValueError(f"unsupported softmax strategy: {strategy}")
    return s


def _gpu_family_key() -> str:
    override = os.getenv("NATTEN_MLX_GPU_FAMILY", "").strip()
    if override:
        return override
    system = platform.system().lower()
    machine = platform.machine().lower()
    if system == "darwin" and ("arm64" in machine or "apple" in machine):
        return "apple_silicon"
    return "apple_unknown"


def _lookup_threadgroup(
    *,
    op: str,
    tokens: int,
    head_dim: int,
    ...
    family = _gpu_family_key()
    table = FORWARD_THREADGROUP_TABLE.get(family) or FORWARD_THREADGROUP_TABLE.get("apple_unknown", {})
    key = (
        op,
        (
            _token_band(tokens),
    ...
    tg = table.get(key)
    return tuple(int(x) for x in tg) if tg is not None else None


def _lookup_softmax_strategy(
    *,
    op: str,
    dtype: mx.Dtype,
    tokens: int,
    ...
    override = _FORWARD_SOFTMAX_STRATEGY_OVERRIDE
    if override in {"stored", "recompute"}:
        return override
    family = _gpu_family_key()
    table = FORWARD_SOFTMAX_STRATEGY_TABLE.get(family) or FORWARD_SOFTMAX_STRATEGY_TABLE.get("apple_unknown", {})
    key = (
        op,
        (
            _dtype_class(dtype),
    ...
    strategy = table.get(key, "stored")
    return _normalize_softmax_strategy(strategy)


def _choose_2d_fused_softmax_strategy(
    *,
    dtype: mx.Dtype,
    out_height: int,
    out_width: int,
    ...
    return _lookup_softmax_strategy(
        op="na2d_fused",
        dtype=dtype,
        tokens=max(out_height * out_width, 1),
        kernel_size=kernel_size,
        causal_rank=int(causal_h) + int(causal_w),
        stride_unit=(int(stride_h) == 1 and int(stride_w) == 1),
    )


def _choose_3d_fused_softmax_strategy(
    *,
    dtype: mx.Dtype,
    out_depth: int,
    out_height: int,
    ...
    return _lookup_softmax_strategy(
        op="na3d_fused",
        dtype=dtype,
        tokens=max(out_depth * out_height * out_width, 1),
        kernel_size=kernel_size,
        causal_rank=int(causal_d) + int(causal_h) + int(causal_w),
        stride_unit=(int(stride_d) == 1 and int(stride_h) == 1 and int(stride_w) == 1),
    )


def _choose_forward_threadgroup_2d(
    *,
    op: str,
    out_height: int,
    out_width: int,
    ...
    if tuned is not None:
        return tuned
    return _threadgroup_2d(out_height, out_width)


def _choose_forward_threadgroup_3d(
    *,
    op: str,
    out_depth: int,
    out_height: int,
...
def _use_vec4_2d_forward(height: int, width: int, head_dim: int) -> bool:
    if head_dim % 4 != 0 or head_dim < 16:
        return False
    return height > 0 and width > 0


def _use_vec4_3d_forward(depth: int, height: int, width: int, head_dim: int) -> bool:
    if head_dim % 4 != 0 or head_dim < 16:
        return False
    return depth > 0 and height > 0 and width > 0
...
def _inverse_map_2d_qk(
    ...
    attn_base = (out_ids * k_area + nbr_ids).astype(np.int32, copy=False)
    query_linear = (
        (out_ids // out_w) * stride_h * width + (out_ids % out_w) * stride_w
    ).astype(np.int32, copy=False)
    query_base = (query_linear * dim).astype(np.int32, copy=False)
    result = (
        mx.array(offsets, dtype=mx.int32),
        _to_compact_index_array(attn_base),
        _to_compact_index_array(query_base),
...
def _inverse_map_3d_qk(
    ...
    attn_base = (out_ids * volume + nbr_ids).astype(np.int32, copy=False)
    out_hw = out_h * out_w
    out_z = out_ids // out_hw
    out_rem = out_ids % out_hw
    out_i = out_rem // out_w
    out_j = out_rem % out_w
    query_linear = (
        ((out_z * stride_d) * height + (out_i * stride_h)) * width + (out_j * stride_w)
    ).astype(np.int32, copy=False)
    query_base = (query_linear * dim).astype(np.int32, copy=False)
    result = (
        mx.array(offsets, dtype=mx.int32),
...
def _get_1d_av_vec4_kernel(kernel_size: int):
    if kernel_size not in _AV_1D_VEC4_KERNELS:
        _AV_1D_VEC4_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na1d_av4_k{kernel_size}",
            input_names=["attention_probs", "value", "dilation_param", "causal_param"],
            output_names=["out"],
            source=source_1d_av_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _AV_1D_VEC4_KERNELS[kernel_size]
...
def _get_2d_qk_vec4_kernel(kernel_size: int):
    if kernel_size not in _QK_2D_VEC4_KERNELS:
        _QK_2D_VEC4_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_qk4_k{kernel_size}",
            input_names=["query", "key", "rpb", "dilation_param", "causal_param"],
            output_names=["out"],
            source=source_2d_qk_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_2D_VEC4_KERNELS[kernel_size]
...
def _get_2d_av_vec4_kernel(kernel_size: int):
    if kernel_size not in _AV_2D_VEC4_KERNELS:
        _AV_2D_VEC4_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_av4_k{kernel_size}",
            input_names=["attention_probs", "value", "dilation_param", "causal_param"],
            output_names=["out"],
            source=source_2d_av_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _AV_2D_VEC4_KERNELS[kernel_size]
...
def _get_3d_qk_vec4_kernel(kernel_size: int):
    if kernel_size not in _QK_3D_VEC4_KERNELS:
        _QK_3D_VEC4_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_qk4_k{kernel_size}",
            input_names=["query", "key", "rpb", "dilation_param", "causal_param"],
            output_names=["out"],
            source=source_3d_qk_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_3D_VEC4_KERNELS[kernel_size]
...
def _get_3d_av_vec4_kernel(kernel_size: int):
    if kernel_size not in _AV_3D_VEC4_KERNELS:
        _AV_3D_VEC4_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_av4_k{kernel_size}",
            input_names=["attention_probs", "value", "dilation_param", "causal_param"],
            output_names=["out"],
            source=source_3d_av_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _AV_3D_VEC4_KERNELS[kernel_size]


def _get_3d_fused_kernel(kernel_size: int, softmax_strategy: str = "stored"):
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, strategy)
    if key not in _FUSED_3D_KERNELS:
        _FUSED_3D_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_fused_k{kernel_size}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_3d_fused(kernel_size, softmax_strategy=strategy),
            ensure_row_contiguous=True,
        )
    return _FUSED_3D_KERNELS[key]


def _get_3d_fused_vec4_kernel(kernel_size: int, softmax_strategy: str = "stored"):
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, strategy)
    if key not in _FUSED_3D_VEC4_KERNELS:
        _FUSED_3D_VEC4_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_fused4_k{kernel_size}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_3d_fused_vec4(kernel_size, softmax_strategy=strategy),
            ensure_row_contiguous=True,
        )
    return _FUSED_3D_VEC4_KERNELS[key]


def _get_3d_fused_causal_kernel(
    kernel_size: int,
    causal_d: int,
    causal_h: int,
    causal_w: int,
    ...
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, int(causal_d), int(causal_h), int(causal_w), strategy)
    if key not in _FUSED_3D_CAUSAL_KERNELS:
        _FUSED_3D_CAUSAL_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_fused_causal_k{kernel_size}_c{causal_d}{causal_h}{causal_w}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_3d_fused_causal(
                kernel_size,
                causal_d=bool(causal_d),
                causal_h=bool(causal_h),
                causal_w=bool(causal_w),
                softmax_strategy=strategy,
            ),
            ensure_row_contiguous=True,
        )
    return _FUSED_3D_CAUSAL_KERNELS[key]


def _get_3d_fused_causal_vec4_kernel(
    kernel_size: int,
    causal_d: int,
    causal_h: int,
    causal_w: int,
    ...
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, int(causal_d), int(causal_h), int(causal_w), strategy)
    if key not in _FUSED_3D_CAUSAL_VEC4_KERNELS:
        _FUSED_3D_CAUSAL_VEC4_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_fused_causal4_k{kernel_size}_c{causal_d}{causal_h}{causal_w}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_3d_fused_causal_vec4(
                kernel_size,
                causal_d=bool(causal_d),
                causal_h=bool(causal_h),
                causal_w=bool(causal_w),
    ...
    return _FUSED_3D_CAUSAL_VEC4_KERNELS[key]
...
def _get_1d_fused_causal_kernel(kernel_size: int):
    if kernel_size not in _FUSED_1D_CAUSAL_KERNELS:
        _FUSED_1D_CAUSAL_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na1d_fused_causal_k{kernel_size}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_1d_fused_causal(kernel_size),
            ensure_row_contiguous=True,
        )
...
def _get_2d_fused_kernel(kernel_size: int, softmax_strategy: str = "stored"):
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, strategy)
    if key not in _FUSED_2D_KERNELS:
        _FUSED_2D_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_fused_k{kernel_size}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_2d_fused(kernel_size, softmax_strategy=strategy),
            ensure_row_contiguous=True,
        )
    return _FUSED_2D_KERNELS[key]


def _get_2d_fused_vec4_kernel(kernel_size: int, softmax_strategy: str = "stored"):
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, strategy)
    if key not in _FUSED_2D_VEC4_KERNELS:
        _FUSED_2D_VEC4_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_fused4_k{kernel_size}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_2d_fused_vec4(kernel_size, softmax_strategy=strategy),
            ensure_row_contiguous=True,
        )
    return _FUSED_2D_VEC4_KERNELS[key]


def _get_2d_fused_causal_kernel(
    kernel_size: int,
    causal_h: int,
    causal_w: int,
    softmax_strategy: str = "stored",
    ...
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, int(causal_h), int(causal_w), strategy)
    if key not in _FUSED_2D_CAUSAL_KERNELS:
        _FUSED_2D_CAUSAL_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_fused_causal_k{kernel_size}_c{causal_h}{causal_w}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
    ...
    return _FUSED_2D_CAUSAL_KERNELS[key]


def _get_2d_fused_causal_vec4_kernel(
    kernel_size: int,
    causal_h: int,
    causal_w: int,
    softmax_strategy: str = "stored",
    ...
    strategy = _normalize_softmax_strategy(softmax_strategy)
    key = (kernel_size, int(causal_h), int(causal_w), strategy)
    if key not in _FUSED_2D_CAUSAL_VEC4_KERNELS:
        _FUSED_2D_CAUSAL_VEC4_KERNELS[key] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_fused_causal4_k{kernel_size}_c{causal_h}{causal_w}_s{strategy}",
            input_names=[
                "query",
                "key",
                "value",
            ...
            output_names=["out"],
            source=source_2d_fused_causal_vec4(
                kernel_size,
                causal_h=bool(causal_h),
                causal_w=bool(causal_w),
                softmax_strategy=strategy,
    ...
    return _FUSED_2D_CAUSAL_VEC4_KERNELS[key]
...
def _get_2d_qk_backward_k_inverse_kernel(kernel_size: int):
    if kernel_size not in _QK_BWD_K_INV_2D_KERNELS:
        _QK_BWD_K_INV_2D_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_qk_bwd_k_inv_k{kernel_size}",
            input_names=[
                "grad_attn",
                "query",
                "inv_offsets",
            ...
            output_names=["out"],
            source=source_2d_qk_backward_k_inverse(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_BWD_K_INV_2D_KERNELS[kernel_size]
...
def _get_2d_qk_backward_q_vec4_kernel(kernel_size: int):
    if kernel_size not in _QK_BWD_Q4_2D_KERNELS:
        _QK_BWD_Q4_2D_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na2d_qk_bwd_q4_k{kernel_size}",
            input_names=[
                "grad_attn",
                "key",
                "stride_param",
            ...
            output_names=["out"],
            source=source_2d_qk_backward_q_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_BWD_Q4_2D_KERNELS[kernel_size]
...
def _get_3d_qk_backward_k_inverse_kernel(kernel_size: int):
    if kernel_size not in _QK_BWD_K_INV_3D_KERNELS:
        _QK_BWD_K_INV_3D_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_qk_bwd_k_inv_k{kernel_size}",
            input_names=[
                "grad_attn",
                "query",
                "inv_offsets",
            ...
            output_names=["out"],
            source=source_3d_qk_backward_k_inverse(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_BWD_K_INV_3D_KERNELS[kernel_size]
...
def _get_3d_qk_backward_q_vec4_kernel(kernel_size: int):
    if kernel_size not in _QK_BWD_Q4_3D_KERNELS:
        _QK_BWD_Q4_3D_KERNELS[kernel_size] = mx.fast.metal_kernel(
            name=f"natten_mlx_na3d_qk_bwd_q4_k{kernel_size}",
            input_names=[
                "grad_attn",
                "key",
                "stride_param",
            ...
            output_names=["out"],
            source=source_3d_qk_backward_q_vec4(kernel_size),
            ensure_row_contiguous=True,
        )
    return _QK_BWD_Q4_3D_KERNELS[kernel_size]
...
def na1d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    if not is_available() or not _supports_1d_fused(kernel_size, stride, dilation):
        raise RuntimeError("nanobind fused 1D forward unsupported configuration")
    ...
    inputs = (
        [q_m, k_m, v_m, stride_param, dilation_param, scale_param]
        if use_causal_kernel
        else [q_m, k_m, v_m, stride_param, dilation_param, causal_param, scale_param]
    )
    out = kernel(
        inputs=inputs,
...
def na1d_forward(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    return _with_fallback(
        lambda: na1d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale),
        lambda: pure.na1d_forward(q, k, v, kernel_size, stride, dilation, is_causal, scale),
    )


def na2d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    if not is_available() or not _supports_2d_fused(kernel_size, stride, dilation):
        raise RuntimeError("nanobind fused 2D forward unsupported configuration")
    ...
    use_causal_kernel = (causal_h == 1) or (causal_w == 1)
    use_vec4 = _use_vec4_2d_forward(height, width, q.shape[-1])
    strategy = _choose_2d_fused_softmax_strategy(
        dtype=q.dtype,
    ...
    if use_vec4 and use_causal_kernel:
        kernel = _get_2d_fused_causal_vec4_kernel(ksize, causal_h, causal_w, strategy)
    elif use_vec4:
        kernel = _get_2d_fused_vec4_kernel(ksize, strategy)
    elif use_causal_kernel:
        kernel = _get_2d_fused_causal_kernel(ksize, causal_h, causal_w, strategy)
    else:
        kernel = _get_2d_fused_kernel(ksize, strategy)
    ...
    tg = _choose_forward_threadgroup_2d(
        op="na2d_fused",
        out_height=out_height,
        out_width=out_width,
        head_dim=q.shape[-1],
        kernel_size=ksize,
        causal_rank=causal_h + causal_w,
        stride_unit=(stride_h == 1 and stride_w == 1),
    )
    out = kernel(
        inputs=inputs,
...
def na2d_forward(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    return _with_fallback(
        lambda: na2d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale),
        lambda: pure.na2d_forward(q, k, v, kernel_size, stride, dilation, is_causal, scale),
    )


def na3d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    if not is_available() or not _supports_3d_fused(kernel_size, stride, dilation):
        raise RuntimeError("nanobind fused 3D forward unsupported configuration")
    ...
    scale_value = (q.shape[-1] ** -0.5) if scale is None else float(scale)
    use_causal_kernel = (causal_d == 1) or (causal_h == 1) or (causal_w == 1)
    use_vec4 = _use_vec4_3d_forward(depth, height, width, q.shape[-1])
    strategy = _choose_3d_fused_softmax_strategy(
        dtype=q.dtype,
        out_depth=out_depth,
        out_height=out_height,
        out_width=out_width,
    ...
    if use_vec4 and use_causal_kernel:
        kernel = _get_3d_fused_causal_vec4_kernel(ksize, causal_d, causal_h, causal_w, strategy)
    elif use_vec4:
        kernel = _get_3d_fused_vec4_kernel(ksize, strategy)
    elif use_causal_kernel:
        kernel = _get_3d_fused_causal_kernel(ksize, causal_d, causal_h, causal_w, strategy)
    else:
        kernel = _get_3d_fused_kernel(ksize, strategy)
    ...
    tg = _choose_forward_threadgroup_3d(
        op="na3d_fused",
        out_depth=out_depth,
        out_height=out_height,
        out_width=out_width,
        head_dim=q.shape[-1],
        kernel_size=ksize,
        causal_rank=causal_d + causal_h + causal_w,
        stride_unit=(stride_d == 1 and stride_h == 1 and stride_w == 1),
    )
    out = kernel(
        inputs=inputs,
...
def na3d_forward(q, k, v, kernel_size, stride, dilation, is_causal, scale):
    ...
    if _supports_3d_fused(kernel_size, stride, dilation):
        return _with_fallback(
            lambda: na3d_fused_forward_strict(q, k, v, kernel_size, stride, dilation, is_causal, scale),
...
def na1d_av_forward(attn, v, kernel_size, stride, dilation, is_causal):
    ...
    def _run():
        use_vec4 = _use_vec4_1d_forward(length, v.shape[-1])
        kernel = _get_1d_av_vec4_kernel(ksize) if use_vec4 else _get_1d_av_kernel(ksize)
        attn_full = attn
        if step != 1:
...
def na2d_qk_forward(q, k, kernel_size, stride, dilation, is_causal, scale):
    ...
    def _run():
        use_vec4 = _use_vec4_2d_forward(height, width, q.shape[-1])
        kernel = _get_2d_qk_vec4_kernel(ksize) if use_vec4 else _get_2d_qk_kernel(ksize)
...
def na2d_av_forward(attn, v, kernel_size, stride, dilation, is_causal):
    ...
    def _run():
        use_vec4 = _use_vec4_2d_forward(height, width, v.shape[-1])
        kernel = _get_2d_av_vec4_kernel(ksize) if use_vec4 else _get_2d_av_kernel(ksize)
        attn_full = attn
        ...
        tg = _choose_forward_threadgroup_2d(
            ...
            kernel_size=ksize,
            causal_rank=causal_h + causal_w,
            stride_unit=(step_h == 1 and step_w == 1),
        )
        out = kernel(
            inputs=[attn_m, v_m, dilation_param, causal_param],
            grid=(width, height, batch * heads),
...
def na3d_qk_forward(q, k, kernel_size, stride, dilation, is_causal, scale):
    ...
    def _run():
        use_vec4 = _use_vec4_3d_forward(depth, height, width, q.shape[-1])
        kernel = _get_3d_qk_vec4_kernel(ksize) if use_vec4 else _get_3d_qk_kernel(ksize)
        q_m = _to_metal_3d(_forward_native_input(q))
        k_m = _to_metal_3d(_forward_native_input(k))
...
def na3d_av_forward(attn, v, kernel_size, stride, dilation, is_causal):
    ...
    def _run():
        use_vec4 = _use_vec4_3d_forward(depth, height, width, v.shape[-1])
        kernel = _get_3d_av_vec4_kernel(ksize) if use_vec4 else _get_3d_av_kernel(ksize)
        attn_full = attn
        ...
        tg = _choose_forward_threadgroup_3d(
            ...
            kernel_size=ksize,
            causal_rank=causal_d + causal_h + causal_w,
            stride_unit=(step_d == 1 and step_h == 1 and step_w == 1),
        )
        out = kernel(
            inputs=[attn_m, v_m, dilation_param, causal_param],
            grid=(width, height, batch * heads * depth),
...
def na2d_qk_backward(q, k, grad_attn, kernel_size, stride, dilation, is_causal, scale):
    ...
    def _run():
        ...
        scale_param = mx.array([scale_value], dtype=mx.float32)
        use_vec4_q = (head_dim % 4 == 0) and (head_dim >= 16)
        grad_q_kernel = (
            _get_2d_qk_backward_q_vec4_kernel(kh)
            if use_vec4_q
            else _get_2d_qk_backward_q_kernel(kh)
        )
        grad_q_x = head_dim // 4 if use_vec4_q else width
        grad_q_y = height * width if use_vec4_q else height
        grad_q_tg = (
            _threadgroup_grad_v_2d_vec4(grad_q_x, height * width)
            if use_vec4_q
            else _threadgroup_2d_heavy(height, width)
        )
        grad_q_m = grad_q_kernel(
            inputs=[grad_attn_m, k_m, stride_param, dilation_param, causal_param, scale_param],
            grid=(grad_q_x, grad_q_y, batch * heads),
        ...
        grad_k_kernel = _get_2d_qk_backward_k_inverse_kernel(kh)
        grad_k_m = grad_k_kernel(
            inputs=[grad_attn_m, q_m, inv_offsets, inv_attn_base, inv_query_base, scale_param],
            grid=(head_dim, height * width, batch * heads),
            threadgroup=_threadgroup_grad_v(head_dim, height * width),
            output_shapes=[(batch, heads, height, width, head_dim)],
            output_dtypes=[mx.float32],
        )[0]
...
def na3d_qk_backward(q, k, grad_attn, kernel_size, stride, dilation, is_causal, scale):
    ...
    def _run():
        ...
        use_vec4_q = (head_dim % 4 == 0) and (head_dim >= 16)
        grad_q_kernel = (
            _get_3d_qk_backward_q_vec4_kernel(kd)
            if use_vec4_q
            else _get_3d_qk_backward_q_kernel(kd)
        )
        grad_q_x = head_dim // 4 if use_vec4_q else width
        grad_q_y = depth * height * width if use_vec4_q else height
        grad_q_z = batch * heads if use_vec4_q else batch * heads * depth
        grad_q_tg = (
            _threadgroup_grad_v_3d_vec4(grad_q_x, depth * height * width)
            if use_vec4_q
            else _threadgroup_2d_heavy(height, width)
        )
        grad_q_m = grad_q_kernel(
            inputs=[grad_attn_m, k_m, stride_param, dilation_param, causal_param, scale_param],
            grid=(grad_q_x, grad_q_y, grad_q_z),
            threadgroup=grad_q_tg,
        ...
        grad_k_kernel = _get_3d_qk_backward_k_inverse_kernel(kd)
        grad_k_m = grad_k_kernel(
            inputs=[grad_attn_m, q_m, inv_offsets, inv_attn_base, inv_query_base, scale_param],
            grid=(head_dim, depth * height * width, batch * heads),
            threadgroup=_threadgroup_grad_v(head_dim, depth * height * width),
            output_shapes=[(batch, heads, depth, height, width, head_dim)],
            output_dtypes=[mx.float32],
        )[0]
