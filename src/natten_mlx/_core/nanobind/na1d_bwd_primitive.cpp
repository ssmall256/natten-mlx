...
MTL::ComputePipelineState* get_1d_bwd_kernel(const std::string& name) {
  {
    std::lock_guard<std::mutex> lock(v2_1d_bwd_mutex());
    auto it = v2_1d_bwd_cache().find(name);
...
void NA1DBwdAttn::eval_gpu(
  ...
  auto* kernel_fn = get_1d_bwd_kernel(kname);
  auto& dev = mx::metal::device(mx::Device::gpu);
...
std::vector<mx::array> na1d_backward_v2(
  ...
  // Stage 2: softmax backward via MLX ops (lazy)
