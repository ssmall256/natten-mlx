...
MTL::ComputePipelineState* get_v2_1d_kernel(const std::string& name, int kernel_size) {
  std::string cache_key = name + "_k" + std::to_string(kernel_size);
  {
    std::lock_guard<std::mutex> lock(v2_1d_kernel_cache_mutex());
    auto it = v2_1d_kernel_cache().find(cache_key);
  ...
  {
    std::lock_guard<std::mutex> lock(v2_1d_kernel_cache_mutex());
    v2_1d_kernel_cache()[cache_key] = kernel;
  }
  return kernel;
}
...
void NA1DFusedForward::eval_gpu(
  ...
  auto* kernel_fn = get_v2_1d_kernel(kname, kernel_size_);
  auto& dev = mx::metal::device(mx::Device::gpu);
