...
std::mutex& v2_3d_kernel_cache_mutex() {
  static std::mutex m;
  return m;
}

std::unordered_map<std::string, MTL::ComputePipelineState*>& v2_3d_kernel_cache() {
  static std::unordered_map<std::string, MTL::ComputePipelineState*> cache;
  return cache;
}

std::string v2_3d_binary_dir() {
  static std::string dir = []() {
    Dl_info info;
    if (!dladdr(reinterpret_cast<void*>(&v2_3d_binary_dir), &info)) {
      return std::string(".");
    }
...
MTL::ComputePipelineState* get_v2_3d_kernel(const std::string& name) {
  ...
  auto* lib = dev.get_library("natten_nb", v2_3d_binary_dir());
  if (!lib) {
...
void NA3DFusedForward::eval_gpu(
    const std::vector<mx::array>& inputs,
    std::vector<mx::array>& outputs) {
  // Input: [B, ID, IH, IW, H, D] spatial-first
  auto& q = inputs[0];
  auto& k = inputs[1];
  auto& v = inputs[2];
  ...
  std::string kname = vec4
      ? ("na3d_fused_v2_stored_vec4_" + suffix)
      : ("na3d_fused_v2_stored_" + suffix);

  NA3DParamsV2 params{
      B, ID, IH, IW, H, D, kernel_size_,
      stride_d_, stride_h_, stride_w_,
      dilation_d_, dilation_h_, dilation_w_,
      causal_d_, causal_h_, causal_w_, scale_};

  size_t out_bytes = static_cast<size_t>(B) * out_d * out_h * out_w * H * D *
...
mx::array na3d_fused_forward_v2(
  ...
  auto prim = std::make_shared<NA3DFusedForward>(
      stream, kernel_size,
      stride_d, stride_h, stride_w,
      dilation_d, dilation_h, dilation_w,
      causal_d ? 1 : 0, causal_h ? 1 : 0, causal_w ? 1 : 0,
      scale, use_vec4);

  mx::Shape out_shape = {B, out_d, out_h, out_w, H, D};
