    ...
    def _na1d_vjp(primals, cotangent, output):
        ...
        try:
            ...
            if fast_metal._can_use_fused_simd(q.shape[-1]):
                grad_q, grad_k, grad_v = fast_metal.na1d_fused_simd_backward(
...
if hasattr(mx, "custom_function"):
    ...
    @mx.custom_function
    def _na1d_varlen_custom(q, k, v, seq_lens, kernel_size_tuple, dilation_tuple, scale_float):
        try:
            from natten_mlx._core import fast_metal
            if fast_metal.is_available():
                return fast_metal.na1d_varlen_forward(
                    q, k, v, seq_lens, kernel_size_tuple, dilation_tuple, scale_float)
        except Exception:
            pass
        from natten_mlx._core import pure
        return pure.na1d_varlen_forward(q, k, v, seq_lens, kernel_size_tuple, dilation_tuple, scale_float)

    @_na1d_varlen_custom.vjp
    def _na1d_varlen_vjp(primals, cotangent, output):
        q, k, v, seq_lens, kernel_size_tuple, dilation_tuple, scale_float = primals
        from natten_mlx._core import pure
        grad_q, grad_k, grad_v = pure.na1d_varlen_backward(
            q, k, v, cotangent, seq_lens, kernel_size_tuple, dilation_tuple, scale_float,
        )
        return (grad_q, grad_k, grad_v, None, None, None, None)

else:
    _na1d_varlen_custom = None


def na1d_varlen_with_grad(q, k, v, seq_lens, kernel_size, dilation, scale):
    """Autograd-capable entrypoint for variable-length 1D NA."""
    if _na1d_varlen_custom is None:
        from natten_mlx._core import pure
        return pure.na1d_varlen_forward(q, k, v, seq_lens, kernel_size, dilation, scale)
    return _na1d_varlen_custom(q, k, v, seq_lens, kernel_size, dilation, scale)
